...
const isContentScript = document.getElementById('discussify-host');

const transform = !isContentScript ?
    (cssText) => cssText :
    (() => {
        const fontSize = parseFloat(window.getComputedStyle(document.documentElement).fontSize) || 10;
        const fontSizeRemRatio = 10 / fontSize;

        return (cssText) => cssText.replace(/\b(\d+(?:\.\d+)?)rem\b/gm, `calc($1rem * ${fontSizeRemRatio})`);
    })();
